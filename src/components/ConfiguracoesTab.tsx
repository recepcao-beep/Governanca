    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="rounded-xl border border-orange-100 bg-orange-50 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="flex items-center gap-2 text-xl font-bold text-orange-900">
          ...
          {!isEditingItems ? (
            <button 
              ...
              className="rounded-lg bg-orange-100 px-4 py-2 text-sm font-bold text-orange-700 hover:bg-orange-200"
            >
              Editar Itens
            </button>
          ) : (
        ...
        {isEditingItems && (
          <div className="mb-4 flex gap-2">
            <input 
              ...
              className="flex-1 rounded-md border border-gray-300 p-2 text-sm focus:border-orange-500 focus:ring-orange-500"
            />
            <button 
        ...
        <div className="flex flex-wrap gap-2">
          {(isEditingItems ? localItems : (requestableItems || [])).map(item => (
            <div key={item} className="flex items-center gap-2 rounded-full border border-orange-200 bg-white px-3 py-1.5 text-sm font-medium text-orange-800 shadow-sm">
              <span>{item}</span>
              {isEditingItems && (
      ...
      <div className="rounded-xl border border-purple-100 bg-purple-50 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {!isEditingPacks ? (
            <button 
              ...
              className="rounded-lg bg-purple-100 px-4 py-2 text-sm font-bold text-purple-700 hover:bg-purple-200"
            >
              Editar Tamanhos
            </button>
          ) : (
        ...
        <p className="mb-6 text-sm text-purple-800">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-lg bg-white p-4 shadow-sm border border-purple-100">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Lençol de Casal</label>
            {isEditingPacks ? (
              <input 
                type="number" 
                value={localPackSizes.lencolCasal}
                onChange={(e) => setLocalPackSizes({...localPackSizes, lencolCasal: Number(e.target.value)})}
                className="w-full rounded-md border border-gray-300 p-2 text-lg font-bold text-gray-900 focus:border-purple-500 focus:ring-purple-500"
              />
            ) : (
              <div className="text-2xl font-black text-purple-700">{packSizes.lencolCasal} <span className="text-sm font-normal text-gray-500">un/pacote</span></div>
            )}
          </div>
          <div className="rounded-lg bg-white p-4 shadow-sm border border-purple-100">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Lençol de Solteiro</label>
            {isEditingPacks ? (
              <input 
                type="number" 
              ...
              <div className="text-2xl font-black text-purple-700">{packSizes.lencolSolteiro} <span className="text-sm font-normal text-gray-500">un/pacote</span></div>
            )}
          </div>
          <div className="rounded-lg bg-white p-4 shadow-sm border border-purple-100">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Fronhas</label>
            {isEditingPacks ? (
              <input 
                type="number" 
              ...
              <div className="text-2xl font-black text-purple-700">{packSizes.fronhas} <span className="text-sm font-normal text-gray-500">un/pacote</span></div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 p-6">
        ...
        <div className="rounded-lg bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-sm font-bold text-gray-700">Status da Sincronização</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className={`font-semibold ${
                ...
                syncStatus.status === 'error' ? 'text-red-700' : 'text-blue-700'
              }`}>
                {syncStatus.message}
              </p>
              {syncStatus.time && (
                <p className="text-xs text-gray-500">Última tentativa: {syncStatus.time}</p>
              )}
            </div>
          </div>

          {syncStatus.debug && (
            <div className="mt-4 rounded bg-gray-100 p-3 text-[11px] font-mono text-gray-700">
              <strong className="block mb-1 text-gray-900">Debug (Primeiros 5 quartos lidos):</strong>
              {syncStatus.debug.split(' | ').map((log, i) => (
                <div key={i} className="border-b border-gray-200 last:border-0 py-1">{log}</div>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h3 className="mb-3 text-lg font-bold text-gray-800">Como funciona a integração?</h3>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h3 className="mb-3 text-lg font-bold text-gray-800">Gerenciar Apartamentos</h3>
        ...
        <div className="space-y-4">
          {floors.map(floor => (
            <div key={floor} className="rounded-lg border border-gray-100 bg-gray-50 p-4">
              <h4 className="mb-3 font-bold text-gray-700">Andar {floor}</h4>
              <div className="flex flex-wrap gap-2">
                {rooms
                  ...
                  .map(room => (
                    <div 
                      key={room.id} 
                      className="flex items-center gap-1.5 rounded-md border border-gray-200 bg-white px-2.5 py-1.5 text-sm font-medium text-gray-700 shadow-sm"
                    >
                      <span>{room.id}</span>
                      <button 
                        onClick={() => setRoomToDelete(room.id)}
                        className="ml-1 rounded-full p-0.5 text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mb-6 text-gray-600">
              Tem certeza que deseja excluir o apartamento <strong className="text-gray-900">{roomToDelete}</strong>? Esta ação não pode ser desfeita.
            </p>
            <div className="flex gap-3">
              <button 
                onClick={() => setRoomToDelete(null)} 
                className="flex-1 rounded-xl bg-gray-100 py-3 font-semibold text-gray-600 hover:bg-gray-200 transition-colors"
              >
                Cancelar
              </button>
              <button 
