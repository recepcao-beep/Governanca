...
export default function PedidosTab({ user }: { user: any }) {
  const { orders, rooms, updateOrderStatus, packSizes, requestableItems, createOrder } = useSocket();
  const [activeSubTab, setActiveSubTab] = useState<'pedidos' | 'enxoval'>('pedidos');
      ...
      <div className="flex rounded-xl bg-gray-200 p-1">
        <button
          ...
          className={`flex-1 rounded-lg py-2 text-sm font-bold transition-all ${
            activeSubTab === 'pedidos' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 rounded-lg py-2 text-sm font-bold transition-all ${
            activeSubTab === 'enxoval' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
          }`}
        >
          Resumo de Enxoval
        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900">Novo Pedido</h3>
              <button onClick={() => setIsCreatingOrder(false)} className="text-gray-500 hover:text-gray-700">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-sm font-bold text-gray-700">Quarto</label>
                <select 
                  value={newOrderRoom}
                  onChange={(e) => setNewOrderRoom(e.target.value)}
                  className="w-full rounded-lg border border-gray-300 p-2 text-gray-900 focus:border-blue-500 focus:ring-blue-500"
                >
              ...
              <div>
                <label className="mb-2 block text-sm font-bold text-gray-700">Itens Disponíveis</label>
                <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto p-1">
                  {(requestableItems || []).map(item => (
                    <button
                      key={item}
                      onClick={() => handleAddItemToNewOrder(item)}
                      className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-700 hover:bg-blue-100"
                    >
                      + {item}
                    </button>
                  ))}
                  {(!requestableItems || requestableItems.length === 0) && (
                    <p className="text-sm text-gray-500 italic w-full">Nenhum item cadastrado nas configurações.</p>
                  )}
                </div>
              </div>
              ...
              {newOrderItems.length > 0 && (
                <div className="rounded-lg bg-gray-50 p-4 border border-gray-200">
                  <h4 className="mb-2 text-sm font-bold text-gray-700">Itens Selecionados</h4>
      ...
      {activeSubTab === 'enxoval' ? (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex items-center gap-3 text-blue-600">
            <BedDouble size={28} />
            <h3 className="text-xl font-bold text-gray-800">Enxoval Necessário</h3>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="flex flex-col items-center justify-center rounded-xl bg-blue-50 p-6 border border-blue-100">
              <span className="text-4xl font-black text-blue-700">{Math.ceil(totalLencolCasal / packSizes.lencolCasal)}</span>
              <span className="mt-2 text-sm font-bold text-blue-900 uppercase tracking-wider text-center">Pacotes de Casal</span>
              <span className="mt-1 text-xs text-blue-600 text-center">{totalLencolCasal} unidades ({packSizes.lencolCasal} un/pacote)</span>
            </div>
            ...
            <div className="flex flex-col items-center justify-center rounded-xl bg-purple-50 p-6 border border-purple-100">
              <span className="text-4xl font-black text-purple-700">{Math.ceil(totalFronhas / packSizes.fronhas)}</span>
              <span className="mt-2 text-sm font-bold text-purple-900 uppercase tracking-wider text-center">Pacotes de Fronhas</span>
              <span className="mt-1 text-xs text-purple-600 text-center">{totalFronhas} unidades ({packSizes.fronhas} un/pacote)</span>
            </div>
          </div>
          
          <div className="mt-6 rounded-lg bg-gray-50 p-4 text-sm text-gray-600 space-y-2">
            ...
            <p className="pt-2 border-t border-gray-200 mt-2"><strong>Embalagens (Configurável):</strong></p>
            <ul className="list-disc pl-5 space-y-1">
        ...
        <>
          {sortedOrders.length === 0 ? (
            <div className="flex h-64 flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 text-center text-gray-500">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {sortedOrders.map(order => {
                ...
                return (
                  <div key={order.id} className="flex flex-col rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                    <div className="mb-3 flex items-center justify-between border-b border-gray-100 pb-3">
                      <div className="flex items-center gap-2">
                        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-100 font-bold text-blue-700">
                          {order.roomId}
                        </span>
                      ...
                      <div className="mt-auto flex flex-col gap-2 pt-3 border-t border-gray-100">
                        {isDnd ? (
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-center gap-1.5 rounded-lg bg-red-50 p-2 text-xs font-medium text-red-700 border border-red-100">
                              <BellOff size={14} /> Quarto em Não Perturbe
                            </div>
                            <button
                              onClick={() => updateOrderStatus(order.id, 'nao_perturbe')}
                              className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-gray-100 py-2 text-xs font-bold text-gray-700 hover:bg-gray-200 transition-colors"
                            >
                          ...
                          <div className="flex flex-col gap-2">
                            <div className="flex gap-2">
                              <button
                                ...
                                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-green-100 py-2 text-xs font-bold text-green-700 hover:bg-green-200 transition-colors"
                              >
                                <CheckCircle2 size={14} /> Entregue
                              </button>
                              <button
                                onClick={() => updateOrderStatus(order.id, 'nao_entregue')}
                                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-red-100 py-2 text-xs font-bold text-red-700 hover:bg-red-200 transition-colors"
                              >
                            ...
                            <button
                              onClick={() => updateOrderStatus(order.id, 'nao_perturbe')}
                              className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-gray-100 py-1.5 text-xs font-bold text-gray-700 hover:bg-gray-200 transition-colors"
                            >
...
function StatusBadge({ status }: { status: string }) {
  const config = {
    pendente: { color: 'bg-yellow-100 text-yellow-800', icon: <Clock size={12} />, label: 'Pendente' },
    entregue: { color: 'bg-green-100 text-green-800', icon: <CheckCircle2 size={12} />, label: 'Entregue' },
    nao_entregue: { color: 'bg-red-100 text-red-800', icon: <XCircle size={12} />, label: 'Não Entregue' },
    nao_perturbe: { color: 'bg-gray-100 text-gray-800', icon: <BellOff size={12} />, label: 'Não Perturbe' },
  };
